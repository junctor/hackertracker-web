          ...
          {filtered.map((p) => (
            <li key={p.id} className="h-full">
              <Link
                ...
                className="to-gray-750 block h-full rounded-2xl border border-gray-700 bg-gradient-to-br from-gray-800 shadow transition-all hover:-translate-y-0.5 hover:shadow-md focus:ring-2 focus:ring-indigo-500 focus:outline-none"
              >
...
function EmptyState({ query }: { query: string }) {
  return (
    <div className="mx-auto max-w-md rounded-2xl border border-dashed border-gray-700 bg-gray-900/40 p-10 text-center">
      ...
      {query ? (
        <p className="mt-1 text-sm text-gray-400">Try a shorter query or check your spelling.</p>
      ) : (
